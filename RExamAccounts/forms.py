...
class ProfileEdit(UserChangeForm):
    class Meta:
        model = UserProfile
        fields = ('username', 'email', 'first_name', 'last_name', 'middle_name', 'study_group')
        widgets = {
            'username': TextInput(attrs={'class': 'form-control', 'placeholder': 'Имя пользователя', 'disabled': ''}),
            'email': EmailInput(attrs={'class': 'form-control', 'placeholder': 'E-Mail', 'disabled': ''}),
